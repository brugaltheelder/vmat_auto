...
class conformal_arc(model_base):
    ...
    def optimize(self, startingX=None, UB=None, display=False, ftol=1e-5, gtol=1e-5):
        ...
        self.dose_dict[self.run_title] = [np.copy(self.current_dose_per_structure[s]) for s in
                                          range(len(self.data.structures))]
        self.obj_dict[self.run_title] = self.current_obj

    def calc_dose_from_variables(self, x=None):
        self.current_dose_per_structure = [np.zeros(self.data.structures[s].num_vox) for s in
                                           range(len(self.data.structures))]
...
class vmat_mip(model_base):
    ...
    def build_model(self):
        ...
        self.generate_num_apertures_per_cp()

        print 'Building aperture network constraint'
    ...
    def generate_dose_variables(self):
        self.dose_var = [self.m.addVars(self.data.structures[s].num_vox,
                                        name='z_{}_'.format(self.data.structures[s].name.replace(' ', '_'))) for s in
                         range(len(self.data.structures))]
        self.m.update()

    def generate_aper_variables(self):
        self.aper_intensity_var = [
            [self.m.addVar(lb=0, name='y_{}_{}'.format(cp, a)) for a in range(len(self.apertures_per_cp[cp]))] for cp in
            range(self.data.num_control_points)]
        self.aper_binary_var = [[self.m.addVar(lb=0, vtype=grb.GRB.BINARY, name='x_{}_{}'.format(cp, a)) for a in
                                 range(len(self.apertures_per_cp[cp]))] for cp in range(self.data.num_control_points)]
        self.m.update()

    def generate_thresholds(self):
        self.thresholds = [[self.data.structures[s].rx for v in range(self.data.structures[s].num_vox)] for s in
                           range(len(self.data.structures))]
        self.m.update()

    def generate_objective_variables(self):
        self.obj_var = [self.m.addVars(self.data.structures[s].num_vox,
                                       name='h_{}_'.format(self.data.structures[s].name.replace(' ', '_'))) for s in
                        range(len(self.data.structures))]
        self.m.update()
    ...
    def build_dose_constraints(self):
        for s in range(len(self.data.structures)):
            ...
            for v in range(self.data.structures[s].num_vox):
                ...
                self.m.addConstr(self.dose_var[s][v], grb.GRB.EQUAL, lin1,
                                 name='Dose_Constraint_{}'.format(self.data.structures[s].name.replace(' ', '_')))

        self.m.update()

    # Select only one aperture per beam
    def generate_num_apertures_per_cp(self):
        for cp in range(self.data.num_control_points):
            self.m.addConstr(sum(self.aper_binary_var[cp][a] for a in range(len(self.apertures_per_cp[cp]))),
                             grb.GRB.EQUAL, self.model_params['aper_limit'], name='One_Aperture_Per_Beam_{}'.format(cp))
        self.m.update()

    def generate_bilinear_constraints(self):
        for cp in range(self.data.num_control_points):
            for a in range(len(self.apertures_per_cp[cp])):
                self.m.addConstr(self.aper_intensity_var[cp][a], grb.GRB.LESS_EQUAL,
                                 self.aper_binary_var[cp][a] * self.model_params['max_intensity'],
                                 name='Bilinear_Upper_Bound_[{}][{}]'.format(cp, a))
                self.m.addConstr(self.aper_intensity_var[cp][a], grb.GRB.GREATER_EQUAL,
                                 self.aper_binary_var[cp][a] * self.model_params['min_intensity'],
                                 name='Bilinear_Lower_Bound_[{}][{}]'.format(cp, a))
        self.m.update()
        ...
        for s in range(len(self.data.structures)):
            for v in range(self.data.structures[s].num_vox):
                ...
                if self.data.structures[s].is_target:
                    self.m.addConstr(self.obj_var[s][v], grb.GRB.GREATER_EQUAL,
                                     self.thresholds[s][v] - self.dose_var[s][v])
        ...
        for cp in range(self.data.num_control_points - 1):
            for a in range(len(self.apertures_per_cp[cp])):
                ...
                for i in range(len(self.apertures_per_cp[cp + 1])):
                    if not is_connected(self.apertures_per_cp[cp][a], self.apertures_per_cp[cp + 1][i],self.model_params['max_distance_per_cp']):
